...
//////////////////// Whitening + Chroma ////////////////////
// simple spectral whitening: divide by a smoothed (moving average) envelope
function whiten(mags, smoothBins=12){
  const N = mags.length;
  const out = new Float32Array(N);
  let acc=0; const q = smoothBins|0;
  for (let k=0;k<N;k++){
    // sliding window [k-q, k+q]
    let lo = Math.max(1, k-q), hi = Math.min(N-1, k+q), sum=0, cnt=0;
...
function chromaFromSpectrum(mags, sr, nfft){
  ...
  for (let k=1;k<mags.length;k++){
    const f = k*sr/nfft;
    if (f<40 || f>6000) continue; // focus useful band
    let w = 1.0;
    // bass emphasis (helps root detection)
    if (f<200) w *= 1.4;
    // slight roll-off to counter treble dominance
    w *= 1/Math.sqrt(1 + (f/2000)**2);
...
function scoreTemplate(ch, tmpl, rot){
  ...
  // light penalty for energy on non-template bins (prevents everything-major)
  let non=0;
...
function bestChordFromChroma(ch){
  let best = {r:0,q:'',sc:-1};
  for (let r=0;r<12;r++){
    const rootBoost = ch[r]*0.25; // prefer strong root
...
function labelFrame(frame, sr){
  const mags   = fftMagPrewindowed(frame.slice()); // slice for in-place window
  const white  = whiten(mags, 10);
  const chroma = chromaFromSpectrum(white, sr, frame.length);
  return bestChordFromChroma(chroma);
}

//////////////////// Median Smoothing over a small window ////////////////////
function medianSmooth(labels, w=3){
...
self.onmessage = (e) => {
  ...
  const labels = medianSmooth(raw, 5); // small temporal stabilizer
